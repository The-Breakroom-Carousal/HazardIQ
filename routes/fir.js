...
router.post('/', async (req, res) => {
  try {
    ...
    if (!officer_name || !station || !description || !priority) {
      return res.status(400).json({ error: 'All fields are required.' });
    }

    const result = await pool.query(
      'INSERT INTO fir (officer_name, station, description, priority) VALUES ($1, $2, $3, $4) RETURNING *',
      [officer_name, station, description, priority]
    );
...
router.get('/', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM fir ORDER BY id DESC');
